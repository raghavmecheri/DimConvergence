...
def _get_k_neighborhood(dataset, k, radius=False):
    if radius:
        neighbours = NearestNeighbors(radius=k, algorithm='ball_tree', n_jobs=-1).fit(dataset)
        return neighbours

    neighbors = NearestNeighbors(n_neighbors=k, algorithm='ball_tree', n_jobs=-1).fit(dataset)
    return neighbors
